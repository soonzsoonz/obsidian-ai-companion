...
export class FactsFeature {
    ...
    private buildPrompt(entry: string, existing: string, stamp: string): string {
        return [
            ...
            '- Mark each fact with the date it was learned, like: (2026-08-07)',
            '- Preserve existing facts unless the new entry corrects them. If corrected,',
            '  update the fact and note the change.',
            '- Never invent. If the entry does not support a fact, leave it out.',
            '- Group under markdown headings (## People, ## Projects, ## Goals, ## Ongoing issues).',
